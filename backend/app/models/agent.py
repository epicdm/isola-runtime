...
class Agent(Base):
    ...
    agent_type: 'native' (platform-hosted) or 'openclaw' (Edge — runs on tenant's own hardware).
    """
    ...
    # Agent type: 'native' (platform-hosted LLM) or 'openclaw' (Edge — remote OpenClaw bot on tenant hardware)
    ...
    # Container lifecycle fields (set when agent_type='openclaw' runs a managed container)
