...
app.include_router(__import__("app.api.isola_bridge_v2", fromlist=["router"]).router, prefix=settings.API_PREFIX)  # ISOLA GLUE — additive async bridge (not upstream)
__import__("app.api.isola_bridge_v2", fromlist=["install_validation_sanitizer"]).install_validation_sanitizer(app)  # ISOLA GLUE — scoped 422 sanitizer
app.include_router(__import__("app.api.isola_bridge_structured", fromlist=["router"]).router, prefix=settings.API_PREFIX)  # ISOLA GLUE — additive structured bridge (not upstream)
